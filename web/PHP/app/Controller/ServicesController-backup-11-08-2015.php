 ...
 * @return void
 */
	public function driver_ride_tracking(){
 ...
 */
	public function end_trip(){
		...
		//new section
		if($userCommitionAmount>0 && $rideendclicktowise==0){
		...
		if($admincommitionearn>0 && $rideendclicktowise==0){
			if($driver_refferalby_id>0){
		...
		$this->diductridecommitions($totalCommitionAmount,$driver_id,$company_id,$ride_id,$taxicelCommition,1);
	...
	public function diductridecommitions($totalCommitionAmount=0,$user_id=0,$company_id=1,$ride_id=0,$commisionper=0,$user_type=0){
		...
		if($totalCommitionAmount>0 && $user_id>0){
			...
			$reccomment="commision debited for Ride ".$ride_id;
			$this->CommissionPayment->query("INSERT INTO tc_user_ride_commitions(`user_id`,`ride_id`,`amount`,`is_withdrawl`,`crt_date`,`credit_debit`,`reccomment`,`commisionper`) VALUES('".$user_id."','".$ride_id."','".$totalCommitionAmount."','0','".date('Y-m-d')."','2','".$reccomment."','".$commisionper."')");
  ...
  // Customer Start Trip Details Service
	...
	*/
	...
	public function drivercommisiontest(){
		...
		die(json_encode(array('status'=>'1','msg'=>'kkkkk')));
	}
