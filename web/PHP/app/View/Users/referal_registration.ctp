...
<script type="text/javascript">
	...
	function validationCheckHandler(){
		if( $("#referalregis").validation( {errorClass:'validationErr'} ) ) {
			...
			if($(".Epass").val()!=$(".ConPass").val()){
				$('.textbox_error02').html('password and confirm password should be same.')
				return false;
			}
...
<div class="login">
	<h1>Signup to App</h1>
		...
		?>
        <div class="opps">
        	<p style="padding: 10px 0 0 64px;color:green; margin:0;">Thank you for registration</p>
            <div class="clr"></div>
        </div>
		<? }?>
	...
	<form method="post" name="referalregis" action="<?php echo $config['BaseUrl'];?>Users/savereferal_registration" id="referalregis" class="refera">
		...
		<div style="padding-top:20px;">
			<select name="selectName" class="nameuserSelect" id="SelectName" validation="blank|Please select type.">
				<option value="">Select</option>
				<option value="2">Customer</option>
				<option value="1">Driver</option>
			</select>
			<div e_rel="SelectName" class="textbox_error01"></div>
			...
			<input type="password" placeholder="password" id="pAss" name="pass" class="password Epass" validation="blank|Please enter password.">  
			<div e_rel="pAss" class="textbox_error01"></div>
			
			<input type="password" placeholder="confirm password" id="CpAss" name="pass" class="password ConPass" validation="blank|Please confirm password.">  
			<div e_rel="CpAss" class="textbox_error01 textbox_error02"></div>
			
			<input type="text" placeholder="Referrer code" id="" value="<? echo ($refcode!='')?$refcode:'';?>" name="my_refferal_code">
